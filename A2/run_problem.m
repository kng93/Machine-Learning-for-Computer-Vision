...
function [] = run_problem(prob, train_data, test_data, pca_vals, beta_vals)
    ...
    for pca_idx = 1:numel(pca_vals)
        ...
        if (num_pca < size(train_data, 2))
            ...
            for beta = beta_vals
                fprintf('BETA NUM: %f ', beta);
                [mdl, loss, train_loss, full_loss, coeff, mins, ranges]= train_model(prob, train_data, num_pca, beta);

                plot_loss(pca_idx) = loss;
                plot_trainloss(pca_idx) = train_loss;
                % Keep track of the lowest loss and the model associated with it
                if min_vals{1} > loss
                    min_vals = {loss; mdl; num_pca; coeff; full_loss; mins; ranges; beta};
                end
            end
        end
    end
    
    fprintf('PCA NUM FOR TEST: %d, BETA: %f, correct_train (full) = %f ', min_vals{3}, min_vals{8}, min_vals{5});
