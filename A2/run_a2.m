...
for prob = PROBLEMS
    switch prob
        case 1.1 % Run A1 w/ discrimant analysis classifier (cdiscr)
...
function [] = run_problem(prob_num, ...
    ...
    for feature_val = feature_vals
        for bin_val = bin_vals
            for part_val = part_vals
                ...
                [mdl, loss]= train_model(prob_num, train_data, test_data);
                
                % Keep track of the lowest loss and the model associated with it
                if min_vals{1} > loss
                    min_vals = {loss; mdl; test_data; bin_val; ...
                                part_val; feature_val; codewords};
                end

                % Print out intermediate test results
                fprintf('CHECK TEST!! bin_size: %d, part_size: %d, feature_val: %d, ', ...
                        bin_val, part_val, feature_val);
    ...
    % Final test run
    fprintf('FINAL RESULT (bin_val = %d, part_size = %d, feature_val = %d):\n', ...
            min_vals{4}, min_vals{5}, min_vals{6}); 
    test_model(min_vals{3}, min_vals{2});
    
    % Print out codewords
    if size(min_vals{7}, 1) > 0
        patch_size = sqrt(size(min_vals{7},1));
        vl_imarraysc(reshape(min_vals{7}(:,:), patch_size, patch_size, [])) ;
    end
...
function [min_mdl, min_loss] = train_model(prob_num, train_data, test_data)
